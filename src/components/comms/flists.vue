<template>
	<div class="mod-c">
		<div class="search-c">
			...
			<router-link :to='{path: "/fund"}' class="add-a" v-if="tonew">新增产品</router-link>
		...
		<el-table
		    ...
		    <el-table-column
		      	...
		      	<template scope="scope">
		      		<router-link :to='{path: "/fund", query: {id: scope.row.id}}' v-if="tonew">{{scope.row.name}}</router-link>
		      		<a class="btn-a" @click="getcurfund(scope.row)"  v-if="!tonew">{{scope.row.name}}</a>
		      	</template>
		    </el-table-column>
		    <el-table-column
		      	prop="code"
  	...
  	export default{
  		...
  		methods: {
		    ...
		    getLists (){
		    	...
		    	//获取数据列表
		    	var that = this;
		    	var url = gl.domain.funds + "?from=" + (that.currentPage - 1) * that.size + "&display=" + that.display + "&key=" + that.key;
		    	gl.fn.getData({
		    		http: that.$http,
		    		url: url,
		    ...
		    dataConvert (){
		    	for(var i = 0; i < this.tableData.length; i++){
		    		...
		    		this.tableData[i].contactStartDate = gl.fn.dateConvert(this.tableData[i].contactStartDate);
		    	}
		    },
